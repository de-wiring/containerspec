...
Then(/^it should not run privileged$/) do
  part = 'HostConfig'
  key = 'Privileged'
...
Then(/^it should run privileged$/) do
  part = 'HostConfig'
  key = 'Privileged'
  arg = 'false'
  err = (@matching_containers || get_containers).select do |c|
...
Then(/^container should expose port '([^ ]*)'$/) do |arg|
...
Then(/^container should expose port '([^ ]*)' on host port '(.*)'$/) do |arg, arg2|
  arg = "#{arg}/tcp" unless (arg =~ /.*\/tcp/) || (arg =~ /.*\/udp/)
  err = (@matching_containers || get_containers).select do |c|
    cc       = ( (c.json['NetworkSettings'] || {})['Ports'] || {} )
    matching = false
...
Then(/^container should not expose port '(.*)'$/) do |arg|
  arg = "#{arg}/tcp" unless (arg =~ /.*\/tcp/) || (arg =~ /.*\/udp/)
  err = (@matching_containers || get_containers).select do |c|
    cc       = ( (c.json['NetworkSettings'] || {})['Ports'] || {} )
    matching = cc[arg] != nil
